...
@section('header_title', __('panel.edit_role_title'))

@section('content')
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black tracking-tight text-slate-900 dark:text-white uppercase tracking-[0.05em]">{{ __('panel.edit_role') }}</h1>
            <p class="text-slate-500 dark:text-slate-400 mt-2 font-medium">{{ __('panel.edit_role_desc_specific', ['name' => $role->name]) }}</p>
        </div>
    ...
    <form action="{{ route('admin.roles.update', $role->id) }}" method="POST" class="space-y-8">
            ...
            <div class="space-y-8">
                <div class="group">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="name" value="{{ $role->name }}" required class="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl py-5 pl-14 pr-6 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all dark:text-white font-bold text-sm" placeholder="{{ __('panel.role_name_placeholder') }}">
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <label class="relative flex items-center p-5 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 cursor-pointer hover:border-brand-500/50 transition-all group">
                            <div class="flex-1">
                                ...
                                <span class="block text-[10px] text-slate-400 mt-1 font-medium">{{ __('panel.permission_desc', ['permission' => $permission]) }}</span>
                            </div>
                            <div class="relative flex items-center">
                                <input type="checkbox" name="permissions[]" value="{{ $permission }}" {{ in_array($permission, $role->permissions) ? 'checked' : '' }} class="peer sr-only">
        ...
        <div class="flex items-center justify-end space-x-4 pt-4">
            ...
            <button type="submit" class="bg-brand-500 hover:bg-brand-600 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-xl shadow-brand-500/25 active:scale-95">{{ __('panel.update_role_btn') }}</button>
        </div>
    </form>
</div>
@endsection
