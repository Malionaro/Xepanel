...
<div class="space-y-10">
    ...
    <div class="flex items-center p-1.5 glass dark:bg-white/5 border-slate-200 dark:border-white/10 rounded-2xl shadow-sm w-fit">
        ...
        <div class="flex items-center space-x-2 px-4 py-2 rounded-xl bg-brand-500/10 border border-brand-500/20 text-brand-600 dark:text-brand-400">
            ...
            <span class="text-[10px] font-black uppercase tracking-widest">Backups</span>
        </div>
    </div>

    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
        <div>
            <h2 class="text-4xl font-black tracking-tight text-slate-900 dark:text-white leading-tight">Data Sovereignty</h2>
            <p class="text-slate-500 dark:text-slate-400 mt-2 text-lg font-medium">Protect and restore your infrastructure with encrypted snapshots.</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-white/5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 border-b border-slate-100 dark:border-white/5">
                        <th class="p-8">Snapshot Metadata</th>
                        <th class="p-8">File Size</th>
                        <th class="p-8 text-center">Timestamp</th>
                        <th class="p-8 text-right">Protocol</th>
                    </tr>
                </thead>
                            ...
                            <td class="p-8">
                                <div class="flex items-center space-x-5">
                                    ...
                                    <div class="flex flex-col min-w-0">
                                        ...
                                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">GZIP Compression</span>
                                    </div>
                                </div>
                            </td>
                            <td class="p-8">
                            ...
                            <td class="p-8 text-right">
                                <div class="flex items-center justify-end space-x-3">
                                    ...
                                    <form action="{{ route('services.backups.destroy', ['id' => $service->id, 'filename' => $backup['name']]) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="w-10 h-10 rounded-xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-400 hover:text-red-500 transition-all hover:bg-red-500/10 border border-slate-200 dark:border-white/10" onclick="return confirm('CRITICAL: Permanent deletion of archive?')">
                            ...
                            <td colspan="4" class="p-24 text-center">
                                <div class="flex flex-col items-center justify-center space-y-6 opacity-40">
                                    ...
                                    <div class="max-w-xs mx-auto">
                                        <p class="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Archives Offline</p>
                                        <p class="text-sm text-slate-500 font-medium mt-2 leading-relaxed">No redundancy snapshots detected. Initialize a manual archive to secure your state.</p>
                                    </div>
                                </div>
                            </td>
                        </tr>
