...
<div class="space-y-10 pb-20">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-[minmax(320px,420px)_1fr] gap-8 xl:gap-10 items-start">
        <!-- Session Manager -->
        <div class="space-y-6">
            ...
            <div class="space-y-5">
                ...
                @empty
                    <div class="glass dark:bg-dark-card border border-slate-200 dark:border-white/5 p-8 rounded-[2rem] shadow-xl">
                        <div class="w-14 h-14 rounded-2xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-400 border border-slate-200 dark:border-white/5 mb-6">
                            <i data-lucide="monitor-off" class="w-7 h-7"></i>
                        </div>
                        <p class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">No Sessions Found</p>
                        <p class="text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed mt-2">No readable session files are currently available.</p>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="space-y-6 min-w-0">
            ...
            <div class="glass dark:bg-dark-card rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-2xl overflow-hidden">
                    ...
                    <table class="w-full min-w-[760px] text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50/50 dark:bg-white/5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 border-b border-slate-100 dark:border-white/5">
                                <th class="px-6 py-5">Protocol Event</th>
                                <th class="px-6 py-5">Source Origin</th>
                                <th class="px-6 py-5">Identity Agent</th>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                            @forelse($logs as $log)
                                <tr class="group hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <td class="px-6 py-6 max-w-[260px]">
                                        <p class="text-[9px] font-bold text-slate-400 truncate uppercase" title="{{ $log['user_agent'] ?? 'N/A' }}">
                                            {{ $log['user_agent'] ?? 'N/A' }}
                                        </p>
                                    </td>
