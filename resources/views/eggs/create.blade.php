...
@section('header_title', 'Create Egg Template')
...
<div class="max-w-5xl mx-auto space-y-8 pb-20">
    <div class="flex items-center justify-between">
        <div class="flex items-center space-x-2 text-gray-500 dark:text-gray-400">
            ...
            <span class="text-gray-900 dark:text-white font-black">New Template</span>
        </div>
    </div>

    <form id="egg-form" action="{{ route('eggs.store') }}" method="POST" class="space-y-8">
        @csrf
                        ...
                        <div class="flex gap-4">
                            <div class="relative w-16 h-16 shrink-0">
                                ...
                                <button type="button" onclick="openIconPicker()" class="w-full h-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-2xl flex items-center justify-center text-brand-500 hover:border-brand-500 transition-all shadow-inner group">
                                    <i id="current-icon" data-lucide="box" class="w-8 h-8 group-hover:scale-110 transition-transform"></i>
                                </button>
                            </div>
                            <input type="text" name="name" class="flex-1 bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-2xl py-4 px-6 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all dark:text-white font-black text-xl" required placeholder="Name of the service type...">
                        </div>
                    </div>
                    <div class="space-y-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="card bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border p-8 rounded-[2.5rem] shadow-sm">
                ...
                <!-- Docker Config -->
                <div id="docker-config" class="hidden space-y-6">
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Base Docker Image</label>
                        <input type="text" name="docker_image" class="w-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-xl py-3 px-4 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all dark:text-white font-mono text-sm" placeholder="e.g. node:20-alpine">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Default Ports</label>
                            <input type="text" name="docker_ports" class="w-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-xl py-3 px-4 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all dark:text-white font-mono text-sm" placeholder="8080:8080">
                        </div>
                        <div class="space-y-2">
                            <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Network</label>
                            <input type="text" name="docker_network" value="bridge" class="w-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-xl py-3 px-4 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all dark:text-white font-mono text-sm">
                        </div>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Startup Override (Optional)</label>
                        <input type="text" name="docker_start_override" class="w-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-xl py-3 px-4 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all dark:text-white font-mono text-sm" placeholder="node index.js">
                    </div>
                </div>

                <!-- Process Config -->
                <div id="process-config" class="space-y-6">
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Default Start Command</label>
                        <input type="text" name="start_command" class="w-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-xl py-3 px-4 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all dark:text-white font-mono text-sm" placeholder="python3 main.py">
                    </div>
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Default Stop Command</label>
                        <input type="text" name="stop_command" class="w-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-xl py-3 px-4 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all dark:text-white font-mono text-sm" placeholder="kill -9 $PID">
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="number" name="default_ram_mb" value="1024" class="w-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-xl py-3 px-4 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all dark:text-white font-bold" required>
                        </div>
                        <div class="space-y-2">
                            <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">CPU (%)</label>
                            <input type="number" name="default_cpu_percent" value="100" class="w-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-xl py-3 px-4 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all dark:text-white font-bold" required>
                        </div>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Disk Space (MB)</label>
                        <input type="number" name="default_disk_mb" value="5120" class="w-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-xl py-3 px-4 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all dark:text-white font-bold" required>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Deployment Tags</label>
                        <input type="text" name="tags" class="w-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-xl py-3 px-4 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all dark:text-white font-medium" placeholder="Minecraft, Proxy, App">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border p-8 rounded-[2.5rem] shadow-sm">
            ...
            <div id="no-vars-notice" class="py-12 text-center border-2 border-dashed border-gray-100 dark:border-dark-border rounded-3xl">
        ...
        <div class="card bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border overflow-hidden rounded-[2.5rem] shadow-sm flex flex-col">
            ...
            <div id="install-script-editor" class="h-64 bg-[#0d1117] text-sm"></div>
            <textarea name="install_script" id="install_script_hidden" class="hidden"></textarea>
        </div>

        <div class="pt-4">
            <button type="submit" class="w-full bg-brand-500 hover:bg-brand-600 text-white font-black py-5 rounded-[2.5rem] transition-all shadow-xl shadow-brand-500/25 active:scale-[0.98] flex items-center justify-center space-x-3">
                <i data-lucide="save" class="w-6 h-6"></i>
                <span class="text-lg">REGISTER NEW EGG TEMPLATE</span>
            </button>
        </div>
    </form>
</div>
