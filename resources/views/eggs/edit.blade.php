...
@section('header_title', 'Egg-Vorlagen: Bearbeiten')
...
<div class="max-w-5xl mx-auto space-y-10 pb-20">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div>
            <h2 class="text-4xl font-black tracking-tight text-slate-900 dark:text-white uppercase italic tracking-[0.05em]">Modify Egg Protocol</h2>
            <p class="text-slate-500 dark:text-slate-400 mt-2 text-lg font-medium">Update system configurations and operational parameters for <span class="text-brand-500">{{ $egg->name }}</span>.</p>
        </div>
    ...
    <form id="egg-form" action="{{ route('eggs.update', $egg->id) }}" method="POST" class="space-y-10">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div class="glass dark:bg-dark-card border border-slate-200 dark:border-white/5 p-10 rounded-[3.5rem] shadow-xl">
                ...
                <div id="docker-config" class="{{ $egg->type === 'docker' ? '' : 'hidden' }} space-y-8">
                    ...
                    <div class="space-y-3">
                        <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Container Data Path (Inside)</label>
        ...
        <div class="pt-6 flex gap-6">
            <button type="submit" class="flex-1 bg-brand-500 hover:bg-brand-600 text-white font-black py-6 rounded-[2.5rem] transition-all shadow-2xl shadow-brand-500/25 active:scale-[0.98] flex items-center justify-center space-x-4 group/submit">
                ...
                <span class="text-xl uppercase tracking-[0.2em] italic">Commit Protocol Updates</span>
            </button>
